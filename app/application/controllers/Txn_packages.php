...
class Txn_packages extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->load->view('txn_packages',$data);
	}
	...
	function add_package()
	{
		if ($this->input->post('postAction') == "true") {
			...
			$this->form_validation->set_rules('maxAmountLabel', 'maximum amount label', 'required|trim');
			...
			if ($this->form_validation->run()) {
				...
				$maxAmountLabel = $this->security->xss_clean($this->input->post('maxAmountLabel'));
				...
				$data = array(
					...
					'pkg_max_amount_label ' => $maxAmountLabel,
					'pkg_due_day' => $dueDay,
					'pkg_percentage' => $percentage,
					'pkg_creator_id' => $adminId,
				);

              		// check if minimum amount > maximum amount
              if($minAmount >= $maxAmount){
                echo "<script>Swal.fire('minimum amount must be less than maximum amount','please update the from and try again','error')</script>";
                return;
              }
				...
				if ($pkg_add > 0) {
					...
					echo '<script>window.open("'.base_url('txn_packages').'","_self")</script>';
...
?>
				...
				<script>
					$(document).ready(function() {
						...
						var maxAmountLabelErr = "<?php echo form_error('maxAmountLabel') ?>";
						...
						$("#max-amount-label-error").html(maxAmountLabelErr);
...
<?php
	...
	{
		if ($this->input->post('postAction') == "true") {
			$this->form_validation->set_rules('minAmount', 'minimum amount', 'required|trim|numeric|greater_than[0]|max_length[11]');
			$this->form_validation->set_rules('maxAmount', 'maximum amount', 'required|trim|numeric|greater_than[0]|max_length[11]');
			$this->form_validation->set_rules('maxAmountLabel', 'maximum amount label', 'required|trim');
			$this->form_validation->set_rules('dueDay', 'maturity day', 'required|trim|numeric|greater_than[0]|max_length[5]');
			$this->form_validation->set_rules('percentage', 'percentage interest', 'required|trim|numeric|greater_than[0]|max_length[5]');
			...
			if ($this->form_validation->run()) {
				$pkgStatus = $this->security->xss_clean($this->input->post('packageStatus'));
              ...
              if($minAmount >= $maxAmount){
				...
				$pkg_save = $this->admin_model->savePkg($data,$packageId);
				if ($pkg_save > 0) {
					$this->session->set_flashdata('form', "<script>Swal.fire('Package saved successfully.','','success')</script>");
					echo '<script>window.open("'.base_url('txn_packages').'","_self")</script>';
				...
				?>
					<script>
						$(document).ready(function() {
							$("#processing").html("No changes were made please try again.");
							$("#processing<?php echo $packageId ?>").html("");
						});
					</script>
				<?php
				}
...
<?php
	...
	function delete_package()
	{
		if ($this->input->post('postAction') == "true") {		
				...
				$pkg_delete = $this->admin_model->deletePkg($packageId);
				if ($pkg_delete > 0) {
					$this->session->set_flashdata('form', "<script>Swal.fire('Package delete successfully.','','success')</script>");
					echo '<script>window.open("'.base_url('txn_packages').'","_self")</script>';

				} else {
					echo "<script>Swal.fire('Package not deleted','please try again','error')</script>";				
				}
