...
class Txn_deleted_deposit extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->load->view('txn_deleted_deposit', $data);
	}
		...
		if ($response > 0) {
			...
			if ($dataReport->num_rows() > 0) {
				...
				if ($dataReport2->num_rows() > 0) {
					...
					echo '<script>window.open("' . base_url('txn_deleted_deposit') . '","_self")</script>';
					return;
				}
	...
	function deleteTxn()
	{
		$txnId = $this->security->xss_clean($this->input->post('txnId'));
		$txnSenderId = $this->security->xss_clean($this->input->post('depositorId'));
		$response = $this->admin_model->finalDelete($txnId, $txnSenderId);
		if ($response > 0) {
			$this->session->set_flashdata('form', '<script>Swal.fire("Transaction deleted successfully","","info")</script>');
			echo '<script>window.open("' . base_url('txn_deleted_deposit') . '","_self")</script>';
		} else {
			echo '<script>Swal.fire("Operation failed","Please try again","error")</script>';
		}
